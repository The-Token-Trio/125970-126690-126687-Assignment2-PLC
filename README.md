...
- Loop execution with progressive `print()` output
...
The full project report (LaTeX source and compiled PDF) is in `report/`. To recompile:
...
- **Type checking** — `components/type_checker.py` infers variable types from first assignment, validates arithmetic and comparisons, checks control-flow conditions, infers function parameter types from the first call, and infers function return types from `return` statements. Inferred types and function signatures live in the symbol table for this pass only; they are not attached to every runtime value.
- **Static vs runtime** — only programs that pass the type checker are executed. The interpreter does not re-run those typing rules: it evaluates the AST using ordinary Python values (`int`, `float`, `bool`, `str`). Correctness at run time therefore relies on the prior static pass, not on tagging each value with a `LanguageType`.
- **Assignment execution** — the interpreter stores evaluated values in the active lexical environment (nested scopes for blocks and calls). Assignments do not carry a separate per-variable type record at run time.
...
- **`print()` execution** — the built-in print emits the value and a type label (e.g. `29.5 : Float`); the label is derived from the value's Python type, consistent with static checking for programs that reach execution.
