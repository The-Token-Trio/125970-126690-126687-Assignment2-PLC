...
references can be resolved during checking. Parameter types are fixed from the
first call site encountered and held constant; the function body is then
verified with those fixed types. All subsequent calls must supply arguments
that match the established types. For functions that are never called, a
separate post-pass infers types from body usage.
