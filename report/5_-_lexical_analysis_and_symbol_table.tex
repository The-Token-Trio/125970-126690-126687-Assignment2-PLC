...
follow. This serves as the canonical interface between lexical analysis and syntax analysis,
...
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.46\textwidth}}
\toprule
...
Float operators &
\texttt{+.}, \texttt{-.}, \texttt{*.}, \texttt{/.} &
\texttt{PLUS\_DOT}, \texttt{MINUS\_DOT}, \texttt{STAR\_DOT}, \texttt{SLASH\_DOT} \\
Other operators &
\texttt{=}, \texttt{==}, \texttt{!=} &
...
alphanumeric characters and underscores, and then checked against the
\texttt{KEYWORDS} map to determine whether they are reserved words or
user-defined names. Notably, \texttt{true} and \texttt{false} are also entries
in this map, mapping to \texttt{TokenType.BOOL\_LITERAL} rather than a
keyword-specific type. Boolean literals are therefore recognised through the
same keyword-fallback mechanism as reserved words, preventing them from being
scanned as user-defined identifiers. In contrast, \texttt{print} is
\emph{not} reserved: it is scanned as an ordinary \texttt{IDENTIFIER} and later
treated as a built-in callable by the type checker and interpreter.
...
When one of these characters is encountered, the lexer checks the next
character: if it is a \texttt{.} (and the character after that is not a digit),
the dot-suffixed float variant is emitted; otherwise the plain integer operator
is emitted. This lookahead prevents the \texttt{.} in a float literal such as
\texttt{3.14} from being consumed as part of an operator.
...
\small
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.38\textwidth}>{\raggedright\arraybackslash}p{0.31\textwidth}@{}}
\toprule
...
\texttt{KEYWORDS} map & Maps reserved words and Boolean literals to token types & token type
selection \\
...
\subsection{Example symbol table hierarchy}
Figure~\ref{fig:symbol-table-example} illustrates a small two-scope symbol
table after the type checker has processed a program containing two top-level
variables and a function definition.
...
\begin{tabular}{@{}lll@{}}
\multicolumn{3}{l}{\textbf{Global scope}} \\
\midrule
Name  & Kind     & Type \\
\midrule
x     & Variable & Integer \\
...
\begin{tabular}{@{}lll@{}}
\multicolumn{3}{l}{\textbf{Function scope: addf}} \\
\midrule
Name  & Kind      & Type \\
\midrule
a     & Parameter & Float \\
...
\end{tabular}
\end{tcolorbox}
\caption{Example symbol table hierarchy with one global scope and one
function scope.}
\label{fig:symbol-table-example}
\end{figure}

The global scope stores top-level variables and function definitions, while
each function invocation creates a child scope containing parameter bindings
and local variables. Lookup operations recursively search parent scopes when
identifiers are not found in the current environment.
...
during lexical analysis. Accordingly, the type checker initialises a global
symbol table and populates it while traversing the AST, defining variables and
functions, managing nested scopes, and validating type correctness.
...
functions, definitions are pre-registered in the global scope so that forward
references can be resolved during checking. For functions that are called,
parameter types are fixed from the first call site encountered and held
constant; the function body is then verified with those fixed types. All
subsequent calls must supply arguments that match the established types. For
functions that are never called, a separate post-pass infers types from body
usage.
