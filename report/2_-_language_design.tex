...
\textbf{Uppercase} identifiers (e.g.\ \texttt{IF}, \texttt{LPAREN},
\texttt{INT\_LITERAL}) are \emph{terminal token types} exactly as defined in the
Token Grammar (Table~\ref{tab:token-specification}).  They represent a named
token emitted by the lexer, not the raw lexeme string.  For example, the
terminal \texttt{IF} matches the lexeme \texttt{if} (via the
lexer rule \texttt{IF ::= 'if'}), but the grammar rule itself references the
\emph{token type}, not the quoted character sequence.
\textbf{Lowercase} identifiers (e.g.\ \texttt{if\_stmt}, \texttt{block},
\texttt{expr}) are non-terminal symbols.

\subsection{From input string to CFG terminals}
\label{sec:input-to-cfg}
The implementation maintains three distinct layers, which must not be mixed:
\[
\text{input source string} \;\rightarrow\; \text{lexeme strings}
\;\rightarrow\; \text{token types} \;\rightarrow\; \text{CFG productions}
\]
...
\texttt{'{' -> LBRACE},
\texttt{'}' -> RBRACE}.
The lexeme \texttt{print} is \emph{not} a keyword in this language, so it
maps to \texttt{IDENTIFIER}, not to a dedicated \texttt{PRINT} token.
...
\noindent This example shows why a CFG should use token names such as
\texttt{IF} and \texttt{ELSE} as terminals, rather than quoted source strings
such as \texttt{"if"} and \texttt{"else"}.
A complete end-to-end pipeline trace applying these distinctions to a
concrete factorial program --- covering lexical analysis, tokenisation,
CFG parsing, type inference, and interpretation --- is provided in the
supplementary walkthrough document.
...
alone, without inspecting operand types. This implementation adopts a strict
interpretation of the specification's ``no overloading'' requirement.
