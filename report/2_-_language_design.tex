...
In the implementation, if \texttt{bool\_expr} parses an expression and finds
no following \texttt{==} or \texttt{!=} operator, the expression is returned
as-is. The type checker then verifies that its inferred type is
\texttt{Boolean}. This allows the grammar to remain simple while keeping the
