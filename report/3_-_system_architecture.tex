...
The environment grows as the checker visits the AST. On initial binding, a
...
references can be resolved. Parameter types are fixed at the first call site
encountered and held constant for all subsequent calls, which must supply
arguments of the same types. The return type is inferred from the
\texttt{return} statements in the function body, which is verified with the
fixed parameter types. Inconsistent return types are reported as a type error.
...
\begin{table}[H]
\centering
\caption{Static semantics summary: constructs and their typing rules}
\label{tab:static-semantics}
\small
