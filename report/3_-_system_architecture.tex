...
\begin{enumerate}[leftmargin=1.5em, label=\arabic*., itemsep=2pt]
  ...
  \item \textbf{If} \textit{node} is a \textbf{BinaryOp}:
        \begin{itemize}[nosep, leftmargin=1.5em]
          ...
          \item If the operator is \texttt{+}, \texttt{-}, \texttt{*}, or \texttt{/}:\quad
                require both operands to be numeric; infer the result using
                Table~\ref{tab:arithmetic-result-types}.
          \item If the operator is \texttt{==} or \texttt{!=}:\quad
                require both operands to be arithmetic; return \texttt{Boolean}.
  ...
  \item \textbf{If} \textit{node} is a \textbf{FunctionCall}:
        \begin{itemize}[nosep, leftmargin=1.5em]
          \item Infer all argument types.
          \item If this is the first valid call: store the argument types as the
                function's parameter types.
          \item Else: compare argument types with the stored parameter types.
          \item Type-check the function body using a local environment.
          \item Return the inferred function return type.
        \end{itemize}

  \item \textbf{If} \textit{node} is a \textbf{Print} statement:\quad
        infer the expression type; accept any of the four supported types.
\end{enumerate}
...
The language does not support operator overloading. Every operator has a single,
fixed meaning determined solely by the operator symbol. Specifically:
\begin{itemize}
  \item The \texttt{+} operator is not overloaded for string concatenation. Applying \texttt{+} to a \texttt{String} operand is a type error.
  \item The comparison operators \texttt{==} and \texttt{!=} are not overloaded for string or boolean equality. Both operands must be arithmetic.
  \item No mixed Boolean or String arithmetic is permitted.
\end{itemize}
The apparent exception --- that \texttt{Integer + Float} yields \texttt{Float}
--- is standard numeric promotion (widening coercion), not operator overloading.
The \texttt{+} operator retains a single semantic meaning (arithmetic addition on
numbers); only the result type widens to accommodate the wider operand.
Overloading would mean assigning an unrelated meaning to the same symbol, such
as using \texttt{+} for both numeric addition and string concatenation, which
this language explicitly rejects.
Because operators have a single, unambiguous type derivable from their operands,
the compiler can always infer expression types without requiring explicit type
...
is \texttt{Integer}, except for division, which always produces \texttt{Float}
regardless of the operand types. If at least one operand is \texttt{Float} for
\texttt{+}, \texttt{-}, or \texttt{*}, the result is \texttt{Float} --- this is
numeric promotion, not operator overloading, because \texttt{+} retains a single
numeric meaning throughout. Any non-numeric operand causes a type error.

\begin{table}[H]
\centering
\caption{Arithmetic result types by operand combination}
...
\midrule
\texttt{Integer} & \texttt{+}, \texttt{-}, \texttt{*} & \texttt{Integer} & \texttt{Integer} \\
\texttt{Integer} & \texttt{/}                       & \texttt{Integer} & \texttt{Float} \\
\texttt{Float}   & \texttt{+}, \texttt{-}, \texttt{*}, \texttt{/} & \texttt{Integer} & \texttt{Float} \\
\texttt{Integer} & \texttt{+}, \texttt{-}, \texttt{*}, \texttt{/} & \texttt{Float}   & \texttt{Float} \\
...
Unary minus (\texttt{-x}) is desugared by the parser into \texttt{0 - x}. The
type checker therefore applies the standard binary subtraction rule: if
\texttt{x} is \texttt{Integer}, the result is \texttt{Integer}; if \texttt{x}
is \texttt{Float}, the result is \texttt{Float}.

\subsection{Boolean Expressions}
The comparison operators \texttt{==} and \texttt{!=} always produce
\texttt{Boolean}. Both operators require two arithmetic (\texttt{Integer} or
\texttt{Float}) operands; this constraint is a deliberate design choice that
keeps comparison semantics unambiguous. Comparisons involving \texttt{Boolean}
or \texttt{String} operands are rejected by the type checker.
