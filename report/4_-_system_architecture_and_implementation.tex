...
\textit{Ran \textless{}filename\textgreater{}} after a successful run on a loaded script file,
\textit{Ran editor contents} when running the editor buffer directly without
having opened a file, \textit{Run failed} when a pipeline stage raises an
error, and \textit{Output cleared} after the \textit{Clear Output} button is
pressed.
...
\begin{itemize}
  ...
  \item \textit{Parse Tree} --- a grammar-oriented ASCII branch rendering of
        ...
        \texttt{assignment}, \texttt{expr}, \texttt{comparison},
        \texttt{additive}, \texttt{term}, and \texttt{factor};
