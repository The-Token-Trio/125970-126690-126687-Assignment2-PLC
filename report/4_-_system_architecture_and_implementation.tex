...
\texttt{a + b} & \texttt{Integer} \quad (Integer $+$ Integer $\rightarrow$ Integer) \\
\bottomrule
\end{tabular}
\end{table}
...
by the command-line runner, the desktop UI, and the test suite. In this Windows
workspace, \texttt{.\textbackslash.venv\textbackslash Scripts\textbackslash python.exe main.py}
runs a script (or built-in sample) and prints the stages in order: tokens, AST
text, type table, and execution output.
\texttt{.\textbackslash.venv\textbackslash Scripts\textbackslash python.exe ui.py}
runs the Tkinter workbench: the same pipeline is used internally, with each
stage shown in a separate tab so the user can inspect results without altering
the underlying flow.
...
\begin{itemize}
  \item \textit{Execution Output} --- the \texttt{print()} values produced
        by stage~4, formatted as \texttt{value : Type}. Runtime output includes
        both the computed value and its inferred static type, reflecting the
        result of the type-checking phase before interpretation;
...
\begin{subfigure}[t]{0.48\textwidth}
  ...
  \caption{Inline type-error display.}
  \label{fig:ui-error}
\end{subfigure}
\caption{Desktop workbench --- output tabs and error display.}
\label{fig:ui-shots}
\end{figure}
