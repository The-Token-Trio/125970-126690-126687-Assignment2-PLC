...
The system follows a pipeline with five stages, each exposing a labelled output
section through the shared pipeline. Lexical and syntactic processing are
...
\midrule
Lexer        & Unrecognised character (\texttt{\#})   & \texttt{LexError} reported; pipeline halts \\
Parser       & Missing closing \texttt{)}              & \texttt{SyntaxError} reported; pipeline halts \\
...
conditions for control flow, fixes function parameter types from the first call
site and validates all subsequent calls against those types
(or infers from body usage for uncalled functions), and infers function return
types from \texttt{return} statements. Programs that
...
\begin{itemize}
	...
	\item function parameter type fixation at the first call site, with consistent validation of all subsequent calls (or body-based inference for uncalled functions),
