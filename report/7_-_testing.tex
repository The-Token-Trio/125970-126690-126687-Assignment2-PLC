...
\texttt{TypeCheckerTests}  & Static type inference (\texttt{type\_checker.py}) & 13 \\
...
\noindent\textbf{ParserTests} confirm that missing semicolons and unclosed
braces raise \texttt{ParseError}, and that operator precedence and
left-associativity are encoded correctly in the parse tree.

\noindent\textbf{TypeCheckerTests} cover type inference for all four types,
the rule that division always returns \texttt{Float}, float promotion in
mixed arithmetic, Boolean-to-Boolean comparison, and the full set of type
error conditions: string arithmetic, non-Boolean conditions for \texttt{if}
and \texttt{while}, assignment type mismatch, undefined variable, wrong
argument count, and argument type mismatch.
